...
contract Comptroller is ComptrollerInterface, ComptrollerV5Storage, ComptrollerErrorReporter, ExponentialNoError {
    ...
    function updateCompSupplyIndex(address cToken) internal {
        updateMomaSupplyIndex(cToken);
        for (uint i = 0; i < allTokens.length; i++) {
            address token = allTokens[i];
            updateTokenSupplyIndex(token, cToken);
        }
    }
    ...
    function updateCompBorrowIndex(address cToken, Exp memory marketBorrowIndex) internal {
        ...
        for (uint i = 0; i < allTokens.length; i++) {
            address token = allTokens[i];
            updateTokenBorrowIndex(token, cToken, marketBorrowIndex);
        }
    }
    ...
    function distributeSupplierComp(address cToken, address supplier) internal {
        distributeSupplierMoma(cToken, supplier);
        for (uint i = 0; i < allTokens.length; i++) {
            address token = allTokens[i];
            distributeSupplierToken(token, cToken, supplier);
        }
    }

    /**
     * @notice Calculate tokens and COMP accrued by a borrower
    ...
    function distributeBorrowerComp(address cToken, address borrower, Exp memory marketBorrowIndex) internal {
        ...
        for (uint i = 0; i < allTokens.length; i++) {
            address token = allTokens[i];
            distributeBorrowerToken(token, cToken, borrower, marketBorrowIndex);
        }
    }
     ...
     */
    function claimToken(address holder, CToken[] memory cTokens, address[] memory tokens) public {
        address[] memory holders = new address[](1);
        holders[0] = holder;
        claimToken(holders, cTokens, tokens, true, true);
    }
     ...
     */
    function claimMoma(address holder, CToken[] memory cTokens) public {
        address[] memory holders = new address[](1);
        holders[0] = holder;
        claimMoma(holders, cTokens, true, true);
    }
