...
describe('GovernorAlpha#propose/5', () => {
  ...
  beforeAll(async () => {
    ...
    gov = await deploy('GovernorAlpha', [address(0), moma._address, address(0)]);
  });
  ...
  describe("simple initialization", () => {
    ...
    it("This function returns the id of the newly created proposal. # proposalId(n) = succ(proposalId(n-1))", async () => {
      await send(moma, 'transfer', [accounts[2], etherMantissa(400001)]);
      await send(moma, 'delegate', [accounts[2]], { from: accounts[2] });
    ...
    it("emits log with id and description", async () => {
      await send(moma, 'transfer', [accounts[3], etherMantissa(400001)]);
      await send(moma, 'delegate', [accounts[3]], { from: accounts[3] });
      await mineBlock();
