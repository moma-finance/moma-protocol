...
describe('GovernorAlpha#state/1', () => {
  let moma, gov, root, acct, delay, timelock;
  ...
  beforeAll(async () => {
    ...
    gov = await deploy('GovernorAlpha', [timelock._address, moma._address, root]);
    await send(timelock, "harnessSetAdmin", [gov._address])
    await send(moma, 'transfer', [acct, etherMantissa(4000000)]);
    await send(moma, 'delegate', [acct], { from: acct });
  });
  ...
  it("Canceled", async () => {
    await send(moma, 'transfer', [accounts[0], etherMantissa(4000000)]);
    await send(moma, 'delegate', [accounts[0]], { from: accounts[0] });
    await mineBlock()
    ...
    await send(moma, 'delegate', [root], { from: accounts[0] });
