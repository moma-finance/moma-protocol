...
describe('MomaScenario', () => {
  let root, accounts;
  let moma;

  beforeEach(async () => {
    [root, ...accounts] = saddle.accounts;
    moma = await deploy('MomaScenario', [root]);
  });
    ...
    [
      ...
      it(`with ${checkpoints} checkpoints, has ${expectedReads} reads`, async () => {
        ...
        while (remaining > 0) {
          ...
          await moma.methods.generateCheckpoints(amt, offset).send({from: root, gas: 200000000});
          remaining -= amt;
          offset += amt;
        }

        let result = await moma.methods.getPriorVotes(root, 1).send();
