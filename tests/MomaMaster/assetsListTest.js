...
describe('#MomaMaster/assetListTest', () => {
  ...
  beforeEach(async () => {
    ...
    allTokens = [OMG, ZRX, BAT, REP, DAI, SKT] = await Promise.all(
      ['OMG', 'ZRX', 'BAT', 'REP', 'DAI', 'sketch']
        .map(async (name) => makeMToken({momaPool, name, symbol: name, supportMarket: name != 'sketch', underlyingPrice: 0.5}))
    );
  });
  ...
  async function checkMarkets(expectedTokens) {
    for (let token of allTokens) {
      ...
      expect(await call(momaPool, 'checkMembership', [customer, token._address])).toEqual(isExpected);
    }
  }

  async function enterAndCheckMarkets(enterTokens, expectedTokens, expectedErrors = null) {
    const {reply, receipt} = await both(momaPool, 'enterMarkets', [enterTokens.map(t => t._address)], {from: customer});
    const assetsIn = await call(momaPool, 'getAssetsIn', [customer]);
  ...
  async function exitAndCheckMarkets(exitToken, expectedTokens, expectedError = 'NO_ERROR') {
    const {reply, receipt} = await both(momaPool, 'exitMarket', [exitToken._address], {from: customer});
    const assetsIn = await call(momaPool, 'getAssetsIn', [customer]);
  ...
  describe('enterMarkets', () => {
    ...
    it("the market must be listed for add to succeed", async () => {
      ...
      await send(momaPool, '_supportMarket', [SKT._address]);
  ...
  describe('exitMarket', () => {
    it("doesn't let you exit if you have a borrow balance", async () => {
      ...
      await setInterestRateModel(OMG);
    ...
    it("rejects unless redeem allowed", async () => {
      ...
      await setInterestRateModel(BAT);
  ...
  describe('entering from borrowAllowed', () => {
    ...
    it("enters when called by a mtoken", async () => {
    ...
    it("reverts when called by not a mtoken", async () => {
      await expect(
        send(momaPool, 'borrowAllowed', [BAT._address, customer, 1], {from: customer})
      ).rejects.toRevert('revert sender must be mToken');
